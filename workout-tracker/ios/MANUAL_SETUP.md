...
2. **`AppLogic`'s XCTest suite (CI only, no Mac needed).** `App` and
   ...
   tooling; see step 5 below for why linking a package is a one-time manual
   step rather than something scripted). `AppLogic` sidesteps that: it's a
   standalone local Swift package (its own `Package.swift`), and
   `xcodebuild test -scheme AppLogic` builds/tests it directly with no
   Xcode project involved. This is where real logic pulled out of the
   app/widget targets (e.g. `WebViewScrollChrome`, the `MainViewController`
   scroll-indicator config) gets actual regression coverage, run in
   `ios.yml` on every push. New Swift logic should go here, not inlined in
   `App`/`LiveActivityWidget` — see step 5.
...
4. **Manual verification on a real device (see "First run" below).** The
...
## 5. Link `AppLogic` into the `App` target
...
This hasn't been done yet as of this writing — `MainViewController` still
inlines the scroll-indicator config rather than calling
`AppLogic.WebViewScrollChrome.hideNativeIndicators(on:)`. Do the link
above, then update `MainViewController.swift` to call it and delete the
inline duplicate.
